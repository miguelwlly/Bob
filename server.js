...
const express=require('express'),cors=require('cors'),crypto=require('crypto'),bcrypt=require('bcryptjs'),jwt=require('jsonwebtoken');
...
function env(n){if(!process.env[n])throw new Error(`${n} não configurada`);return process.env[n]}
function adminAuth(req,res,next){const h=req.headers.authorization||'',t=h.startsWith('Bearer ')?h.slice(7):null;if(!t)return res.status(401).json({error:'Não autorizado.'});try{const p=jwt.verify(t,env('ADMIN_JWT_SECRET'));if(p.role!=='admin')throw 0;next()}catch{return res.status(401).json({error:'Sessão expirada ou inválida.'})}}
...
}res.sendStatus(200)}catch(e){console.error('Webhook',e);res.sendStatus(500)}});
...
app.patch('/api/admin/orders/:id',adminAuth,(req,res)=>{const allowed=['RECEBIDO','EM PREPARAÇÃO','PRONTO','SAIU PARA ENTREGA','FINALIZADO','CANCELADO'];if(!allowed.includes(req.body?.orderStatus))return res.status(400).json({error:'Status inválido.'});const o=db.updateOrder(req.params.id,{orderStatus:req.body.orderStatus});if(!o)return res.status(404).json({error:'Pedido não encontrado.'});res.json({order:o})});
app.get('/api/admin/config',adminAuth,(q,s)=>s.json(db.readConfig()));
app.put('/api/admin/config',adminAuth,(req,res)=>{const c=db.readConfig(),n={...c,deliveryFee:Number(req.body?.deliveryFee??c.deliveryFee)||0,whatsapp:String(req.body?.whatsapp??c.whatsapp),schedule:String(req.body?.schedule??c.schedule),address:String(req.body?.address??c.address)};db.writeConfig(n);res.json(n)});
app.get('/api/admin/settings/mercadopago',adminAuth,(q,s)=>s.json(settings.publicSettings()));
app.post('/api/admin/settings/mercadopago',adminAuth,async(req,res)=>{try{const c=db.readPaymentSettings(),b=req.body||{},n={...c,enabled:!!b.enabled,environment:b.environment==='production'?'production':'test',publicKey:typeof b.publicKey==='string'?b.publicKey.trim():(c.publicKey||'')};if(typeof b.accessToken==='string'&&b.accessToken.trim()){const t=b.accessToken.trim(),test=await mp.testConnection(t);if(!test.ok)return res.status(400).json({error:'Access Token inválido ou não aceito pela API.'});n.accessTokenEncrypted=enc.encrypt(t);n.accessTokenLast4=enc.last4(t);n.lastTestStatus='connected';n.lastTestAt=new Date().toISOString()}if(typeof b.webhookSecret==='string'&&b.webhookSecret.trim()){n.webhookSecretEncrypted=enc.encrypt(b.webhookSecret.trim());n.webhookSecretLast4=enc.last4(b.webhookSecret.trim())}if(n.enabled&&!n.accessTokenEncrypted)return res.status(400).json({error:'Informe um Access Token antes de ativar o Mercado Pago.'});db.writePaymentSettings(n);res.json(settings.publicSettings())}catch(e){console.error(e);res.status(500).json({error:'Erro ao salvar configurações do Mercado Pago.'})}});
app.post('/api/admin/settings/mercadopago/test',adminAuth,async(req,res)=>{try{const t=settings.getAccessToken();if(!t)return res.json({status:'incomplete',message:'Nenhum Access Token configurado.'});const r=await mp.testConnection(t),st=r.ok?'connected':r.reason==='invalid'?'invalid':'error';const c=db.readPaymentSettings();db.writePaymentSettings({...c,lastTestStatus:st,lastTestAt:new Date().toISOString()});res.json({status:st,message:st==='connected'?'Conexão funcionando.':st==='invalid'?'Credencial inválida.':'Erro ao conectar com o Mercado Pago.'})}catch{res.status(500).json({status:'error',message:'Erro ao testar a conexão.'})}});
